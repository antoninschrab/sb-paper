...
This repository provides the implementation for the following hypothesis tests in [tests.py](tests.py): SB (Single-Step), TB (Two-Batch), SeqSB (Sequential Single-Step) as well as their 'worst-case' variants for minimum (Bonferroni), average and median merging functions. These tests can be used with any statistics (provided by the user), and efficient implementations for HSIC and MMD are provided. It also provides implementation for the TB and SB conformal prediction procedures, presented in [conformal.py](conformal.py). The code is available under the [MIT License](LICENSE.md).
